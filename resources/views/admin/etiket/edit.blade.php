...
@section('content')
    <form action="{{ route('etiket.update', $etiket->id) }}" method="POST" enctype="multipart/form-data" id="form-create">
        @csrf
        @method('put')
        <div class="card">
            <div class="card-body">
                <div class="conteiner-fluid">
                    <div class="row">
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-3">
                                <input type="text" class="form-control @error('nama') is-invalid @enderror"
                                    id="nama" name="nama" value="{{ old('nama', $etiket->nama) }}" readonly>
                                @error('nama')
                                ...
                                <select class="form-select form-select-lg @error('departement_id') is-invalid @enderror"
                                    ...
                                    @foreach ($dept as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('departement_id', $etiket->departement_id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->departement }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                <select class="form-select form-select-lg @error('jenis_bantuan') is-invalid @enderror"
                                    ...
                                    @foreach ($jenis as $item)
                                        <option value="{{ $item }}"
                                            {{ old('jenis_bantuan', $etiket->jenis_bantuan) == $item ? 'selected' : '' }}>
                                            {{ $item }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                <select class="form-select form-select-lg @error('prioritas') is-invalid @enderror"
                                    ...
                                    @foreach ($prioritas as $item)
                                        <option value="{{ $item }}"
                                            {{ old('prioritas', $etiket->prioritas) == $item ? 'selected' : '' }}>
                                            {{ $item }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-8">
                                <input type="text" class="form-control @error('subject') is-invalid @enderror"
                                    id="subject" name="subject" value="{{ old('subject', $etiket->subject) }}">
                                @error('subject')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-8">
                                <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content" rows="5">{{ old('content', $etiket->content) }} </textarea>
                                @error('content')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                ...
                                <div class="col-md-3">
                                    <select class="form-select form-select-lg @error('status') is-invalid @enderror"
                                        ...
                                        @foreach ($status as $item)
                                            <option value="{{ $item }}"
                                                {{ old('status', $etiket->status) == $item ? 'selected' : '' }}>
                                                {{ $item }}
                                            </option>
                                        @endforeach
                                    </select>
