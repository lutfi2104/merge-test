        ...
        <table class="table myTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="text-center" style="background-color: {{ $item->status === 'Passed' ? 'green' : 'yellow' }}; color: {{ $item->status === 'Passed' ? 'white' : 'white' }}; border-radius: 10px; padding: 5px;">
                            {{ $item->status }}
                        </div>
                    </td>
                    ...
                    <td>
                        <div class="d-flex gap-2 justify-content-center">
                            <a class="btn btn-warning text-center" href="{{ route('ujirm.edit', $item->id) }}" role="button">Edit</a>
                            <a class="btn btn-warning text-center" href="{{ route('ujirm.show', $item->id) }}" role="button">Detail</a>
                            @if (auth()->user()->hasRole('Super Admin') ||
                            auth()->user()->hasRole('Admin QC'))
                            <form action="{{ route('ujirm.destroy', $item->id) }}" method="post" id="form_delete{{ $item->id }}">
                                @csrf
                                @method('delete')
                                <button type="submit" class="btn btn-danger" onclick="return hapus_data('#form_delete{{ $item->id }}')">Hapus</button>
                            </form>
                            @endif
                        </div>
                    </td>
